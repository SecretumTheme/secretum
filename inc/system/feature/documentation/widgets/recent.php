 ...
 * Displays recent updates from the 'documention' post_type
 *
 * @package WordPress
...
if(!class_exists('SecretumRecentWidget'))
{
	class SecretumRecentWidget extends WP_Widget
	{
		...
		function __construct()
		{
			parent::__construct(
				// Widget Base ID
				'secretum_documentation_recent_widget',

				// Widget Name
				__('Documentation Recent Updates', 'secretum'),

				// Widget Options
				array(
					'description' 	=> __('Set the number of recent Documentation updates to dipslay.', 'secretum'),
					'classname' 	=> 'documentation_recent_widget'
				)
			);

			// Set Widget Input Title
			$this->widget_title = __('Documentation Updates', 'secretum');
		}
