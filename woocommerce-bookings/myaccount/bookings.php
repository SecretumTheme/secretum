	...
	foreach($tables as $table) { ?>
		...
		<table class="my_account_bookings table table-bordered table-hover">
			<thead class="thead-dark">
				<tr>
					<th scope="col" class="booking-id"><?php _e('ID', 'secretum'); ?></th>
					<th scope="col" class="booked-product"><?php _e('Booked', 'secretum'); ?></th>
					<th scope="col" class="order-number"><?php _e('Order', 'secretum'); ?></th>
					<th scope="col" class="booking-start-date"><?php _e('Start Date', 'secretum'); ?></th>
					<th scope="col" class="booking-end-date"><?php _e('End Date', 'secretum'); ?></th>
					<th scope="col" class="booking-status"><?php _e('Status', 'secretum'); ?></th>
		...
		<div class="woocommerce-pagination woocommerce-pagination--without-numbers woocommerce-Pagination">
			<?php if (1 !== $page) { ?>
				<a class="woocommerce-button woocommerce-button--previous woocommerce-Button woocommerce-Button--previous button" href="<?php echo esc_url(wc_get_endpoint_url('bookings', $page - 1)); ?>"><?php _e('Previous', 'secretum'); ?></a>
			<?php } ?>
			<?php if ($count >= $bookings_per_page) { ?>
				<a class="woocommerce-button woocommerce-button--next woocommerce-Button woocommerce-Button--next button" href="<?php echo esc_url(wc_get_endpoint_url('bookings', $page + 1)); ?>"><?php _e('Next', 'secretum'); ?></a>
			<?php } ?>
		</div>
