...
if ( ! empty( $tables ) ) {
	foreach ( $tables as $secretum_table ) { ?>
		<h2><?php echo esc_html( $secretum_table['header'] ); ?></h2>
		...
		<table class="my_account_bookings table table-bordered table-hover">
			...
			<tbody>
				<?php foreach ( $secretum_table['bookings'] as $secretum_booking ) { ?>
					<?php $secretum_count++; ?>
					<tr>
						<td class="booking-id"><?php echo esc_html( $secretum_booking->get_id() ); ?></td>
						<td class="booked-product">
							<?php if ( $secretum_booking->get_product() && $secretum_booking->get_product()->is_type( 'booking' ) ) { ?>
							<a href="<?php echo esc_url( get_permalink( $secretum_booking->get_product()->get_id() ) ); ?>">
								<?php echo esc_html( $secretum_booking->get_product()->get_title() ); ?>
							</a>
							<?php } ?>
						</td>
						<td class="order-number">
							<?php if ( $secretum_booking->get_order() ) { ?>
								<a href="<?php echo esc_url( $secretum_booking->get_order()->get_view_order_url() ); ?>">
									<?php echo esc_html( $secretum_booking->get_order()->get_order_number() ); ?>
								</a>
							<?php } ?>
						</td>
						<td class="booking-start-date"><?php echo esc_html( $secretum_booking->get_start_date() ); ?></td>
						<td class="booking-end-date"><?php echo esc_html( $secretum_booking->get_end_date() ); ?></td>
						<td class="booking-status"><?php echo esc_html( wc_bookings_get_status_label( $secretum_booking->get_status() ) ); ?></td>
						<td class="booking-cancel">
							<?php if ( 'cancelled' !== $secretum_booking->get_status() && 'completed' !== $secretum_booking->get_status() && ! $secretum_booking->passed_cancel_day() ) { ?>
								<a href="<?php echo esc_url( $secretum_booking->get_cancel_url() ); ?>" class="button cancel"><?php esc_html_e( 'Cancel', 'secretum' ); ?></a>
							<?php } ?>
						</td>
					</tr>
				<?php } ?>
		...
		<div class="woocommerce-pagination woocommerce-pagination--without-numbers woocommerce-Pagination">
			...
			<?php } ?>
			<?php if ( $secretum_count >= $bookings_per_page ) { ?>
		...
		<?php
		do_action( 'woocommerce_after_account_bookings_pagination' );
	}
} else {
	?>
