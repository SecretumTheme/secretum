						...
						<td class="woocommerce-orders-table__cell woocommerce-orders-table__cell-<?php echo esc_attr($column_id); ?> align-middle" data-title="<?php echo esc_attr($column_name); ?>">
							...
							<?php } elseif ('order-number' === $column_id) { ?>
								...
								<a href="<?php echo esc_url($order->get_view_order_url()); ?>">
									...
									<?php echo _x('#', 'hash before order number', 'secretum') . $order->get_order_number(); ?>
							...
							<?php } elseif ('order-total' === $column_id) { ?>
								...
								<?php printf(_n('%1$s for %2$s item', '%1$s for %2$s items', $item_count, 'secretum'), $order->get_formatted_order_total(), $item_count); ?>
	...
	<?php if (1 < $customer_orders->max_num_pages) { ?>
		...
		<div class="woocommerce-pagination woocommerce-pagination--without-numbers woocommerce-Pagination">
			...
			<?php if (1 !== $current_page) { ?>
				...
				<a class="woocommerce-button woocommerce-button--previous woocommerce-Button woocommerce-Button--previous button" href="<?php echo esc_url(wc_get_endpoint_url('orders', $current_page - 1)); ?>"><?php _e('Previous', 'secretum'); ?></a>

			<?php } ?>

			<?php if (intval($customer_orders->max_num_pages) !== $current_page) { ?>

				<a class="woocommerce-button woocommerce-button--next woocommerce-Button woocommerce-Button--next button" href="<?php echo esc_url(wc_get_endpoint_url('orders', $current_page + 1)); ?>"><?php _e('Next', 'secretum'); ?></a>
...
<?php } else { ?>
	...
	<div class="woocommerce-message woocommerce-message--info woocommerce-Message woocommerce-Message--info woocommerce-info p-3">
		...
		<a class="woocommerce-Button btn btn-primary btn-lg" href="<?php echo esc_url(apply_filters('woocommerce_return_to_shop_redirect', wc_get_page_permalink('shop'))); ?>">
			...
			<?php _e('Go shop', 'secretum') ?>

		</a>

		<?php _e('No order has been made yet.', 'secretum'); ?>
